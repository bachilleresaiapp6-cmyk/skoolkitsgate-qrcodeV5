...
import { Loader2, Search, Calendar as CalendarIcon, FilterX, UserCog, ArrowDownUp } from 'lucide-react';
...
export function AttendanceHistoryManager({ idEscuela }: { idEscuela?: string }) {
  ...
  const cleanFormat = (val: any, type: 'date' | 'time') => {
    ...
    if (type === 'time') {
        ...
        return match ? match[1].substring(0, 5) : s;
    }
    return s;
  };
  ...
  const getMovimientoBadge = (movimiento: string) => {
    ...
    return (
        <Badge variant="outline" className={isEntrada ? "border-green-500 text-green-700 bg-green-50" : "border-orange-500 text-orange-700 bg-orange-50"}>
            {isEntrada ? 'ENT' : 'SAL'}
        </Badge>
    );
  };
                        ...
                        <TableRow key={`${record.email}-${record.fecha}-${record.hora}-${index}`} className="hover:bg-muted/20 border-b last:border-0">
                          <TableCell className="px-3 py-3">
                              <div className="flex flex-col">
                                  <span className="font-mono text-[10px] font-black leading-none">{cleanFormat(record.hora, 'time')}</span>
